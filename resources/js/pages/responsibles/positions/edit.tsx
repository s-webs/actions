...
export default function ResponsiblePositionEdit({ position, measures }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Должности и ответственные', href: '/responsibles/accounts' },
        { title: position.name, href: `/responsibles/positions/${position.id}/edit` },
    ];

    const { data, setData, put, processing, errors } = useForm({
        name: position.name,
        measure_ids: position.measure_ids,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('responsibles.positions.update', position.id));
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 p-6">
                <div className="flex items-center justify-between gap-4">
                    <h1 className="text-xl font-medium">Должность</h1>
                            ...
                            {measures.map((measure) => (
                                <label key={measure.id} className="flex items-start gap-2 py-1.5 text-sm">
                                    ...
                                    <span>
                                        ...
                                        {measure.responsible_id && measure.responsible_id !== position.id ? (
